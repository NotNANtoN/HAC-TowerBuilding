        ...
        for i in range(self.FLAGS.layers):
            ...
            # If not highest layer, compare to subgoal thresholds
                ...
                assert len(proj_subgoal) == len(self.goal_array[i]) == len(env.subgoal_thresholds),\
                    "Projected subgoal, actual subgoal, and subgoal thresholds should have same dimensions. projected subgoal: " \
                    + str(np.round(proj_subgoal, 2)) + " goal array[i]: " + str(np.round(self.goal_array[i], 2)) \
                    + " env.subgoal_thresholds: " + str(np.round(env.subgoal_thresholds, 2))
